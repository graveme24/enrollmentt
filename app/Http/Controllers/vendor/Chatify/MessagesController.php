...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        return Response::json([
            'favorite' => $favorite,
            'fetch' => $fetch,
            'user_avatar' => Storage::disk('s3')->url('public/avatar/' . $fetch->avatar),
        ]);
    }
        ...
        if ($request->hasFile('file')) {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed)) {
                    ...
                    $file->storeAs('public/' . '/' .'attachments/', $attachment, 's3');
    ...
    public function search(Request $request)
    {
        ...
        if (Auth::user()==Qs::userIsStudent()) {
            $profiles = User::where('user_type', 'teacher');
            $records = $profiles->where('name', 'LIKE', "%{$input}%");
        ...
        if (Auth::user()==Qs::userIsTeacher()) {
            $profiles = User::where('user_type', 'student')->orWhere('user_type', 'parent');
            $records = $profiles->where('name', 'LIKE', "%{$input}%");
            foreach ($records->get() as $record) {
                $getRecords .= view('Chatify::layouts.listItem', [
                    'get' => 'search_item',
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    if (Auth::user()->avatar != config('chatify.user_avatar.default')) {
                        $path = storage_path('public/' . '/' . 'avatar/' . Auth::user()->avatar , 's3');
                        if (file_exists($path)) {
                            @unlink($path);
                        }
                    }
